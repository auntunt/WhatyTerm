...
// ── 速率限制 ──────────────────────────────────────────────────────
app.use('/api/auth/', authLimiter);               // 认证：每分钟 10 次
app.use('/api/sessions/:sessionId/analyze-now', aiLimiter);  // AI 分析：每分钟 30 次
app.use('/api/', apiLimiter);                     // 其余 API：每分钟 120 次
...
// ── ProviderHelper（getCurrentProvider / getAllProviders / CLAUDE_PROVIDER_PRIORITY）
import { createProviderHelper, getAllProviders, CLAUDE_PROVIDER_PRIORITY, loadProviderPriority } from './services/ProviderHelper.js';
// providerHelper 实例在服务启动后（sessionManager/sessionRelay 初始化完成后）创建
// 见下方 _initProviderHelper() 调用
let providerHelper = null;
function _initProviderHelper() {
  providerHelper = createProviderHelper({
    sessionManager, sessionRelay, maskApiKey,
    readClaudeProcessEnv, getClaudeProcStartEpoch,
    getTmuxPrefix, execAsync
  });
}
// 向后兼容：让原有调用 getCurrentProvider(...) 的代码继续工作
function getCurrentProvider(appType, workingDir = null, tmuxSessionName = null) {
  return providerHelper.getCurrentProvider(appType, workingDir, tmuxSessionName);
}
...
// ── Claude Code 配置路由 + 供应商优先级路由 ──────────────────
import { createClaudeConfigRouter, createProviderPriorityRouter } from './routes/claudeConfigRoutes.js';
// 这两个 router 依赖 providerHelper，在 _initProviderHelper() 后创建
// 用懒初始化中间件包装，确保 providerHelper 已就绪
app.use('/api/claude-code', (req, res, next) => {
  if (!providerHelper) return res.status(503).json({ error: '服务初始化中' });
  if (!app._claudeConfigRouter) {
    app._claudeConfigRouter = createClaudeConfigRouter({ providerHelper, sessionManager, io });
  }
  app._claudeConfigRouter(req, res, next);
});
app.use('/api/provider-priority', (req, res, next) => {
  if (!app._providerPriorityRouter) {
    app._providerPriorityRouter = createProviderPriorityRouter({ PRIORITY_LIST: CLAUDE_PROVIDER_PRIORITY });
  }
  app._providerPriorityRouter(req, res, next);
});

// ── CLI 工具路由 ────────────────────────────────────────────
const cliToolsRouter = createCliToolsRouter({ cliRegistry, cliLearner });
app.use('/api/cli-tools', cliToolsRouter);


// ── Token 统计路由 ──────────────────────────────────────────
const tokenStatsRouter = createTokenStatsRouter(tokenStatsService);
app.use('/api/token-stats', tokenStatsRouter);

// ── 项目录制路由 ────────────────────────────────────────────
const projectRecordingRouter = createProjectRecordingRouter({
  projectRecordingService, terminalRecorder
});
app.use('/api/project-recordings', projectRecordingRouter);

// ── 休眠阻止路由 ────────────────────────────────────────────
const sleepRouter = createSleepRouter({ sleepPrevention });
app.use('/api/sleep-prevention', sleepRouter);
